...
.alert-banner {
    ...
    gap: 12px;
    margin-bottom: 14px;
    animation: alertFlash 1.2s ease-in-out infinite alternate;
}
@keyframes alertFlash {
    from { border-color: #dc2626; }
    to   { border-color: rgba(220,38,38,0.4); }
}
...
.qrow:hover { border-color: rgba(234,88,12,0.3); }
...
.pulse { animation: pulseDot 1.8s ease-in-out infinite; display: inline-block; }
...
@keyframes emergencyFlash { from{opacity:1} to{opacity:0.5} }
...
def plotly_dark(margin=None):
    """Return a base Plotly dark layout dict. margin is NOT included by default
    so callers can pass margin= without triggering a duplicate-keyword error."""
    d = dict(
...
<div style="background:#0b0b0b;border-bottom:1px solid #2a1400;padding:10px 0 6px;margin-bottom:0">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      <span style="padding:3px 10px;border-radius:12px;font-size:8px;font-weight:700;border:1px solid rgba(22,163,74,.3);background:#0a160a;color:{GREEN}">
        ● LIVE
      </span>
      <span style="padding:3px 10px;border-radius:12px;font-size:8px;font-weight:700;border:1px solid rgba(220,38,38,.3);background:#160a0a;color:{RED}">
        ⚠ 3 ALERTS
      </span>
      <span style="padding:3px 10px;border-radius:12px;font-size:8px;font-weight:700;border:1px solid #1e0a00;color:#d97706">
        {new_notifs} NOTIFS
      </span>
      <span style="padding:3px 10px;border-radius:12px;font-size:8px;border:1px solid #1e0a00;color:#4a2a0a;cursor:default">
        {utc_now}
      </span>
    </div>
  </div>
...
if st.session_state.siren:
    st.markdown(
        '<div class="ebar">⚠ SEISMIC EMERGENCY ACTIVE — M7.2 DETECTED — BALI, INDONESIA — TSUNAMI WARNING — SEEK HIGH GROUND ⚠</div>',
        unsafe_allow_html=True
    )
...
ticker_items = " ".join([
    ...
    f'<span style="color:{mag_color(q["mag"])};font-weight:700">M{q["mag"]}</span>'
    f'<span style="color:#5a3a1a">{q["place"]}</span>'
    f'<span style="color:#3a2010">{time_ago(q["time"])}</span>'
    f'</span>'
    for q in QUAKES * 2
])
st.markdown(
...
with tabs[0]:
    ...
    with left:
        ...
        for q in filtered:
            c = mag_color(q["mag"])
            sel = "border-color:" + c if st.session_state.selected_quake["id"] == q["id"] else ""
            st.markdown(f"""
            <div class="qrow" style="{sel}" onclick="">
    ...
    with right:
        ...
        xs = list(range(100))
        wave_y = [math.sin(i*0.5)*14 + math.sin(i*1.3)*6 + random.gauss(0,2.5) for i in xs]
        ...
        for z in RISK_ZONES:
            ...
            <div class="risk-bar-wrap">
              ...
              <div class="risk-bar-bg">
                <div class="risk-bar-fill" style="width:{z['risk']}%;background:{rc}"></div>
              </div>
            </div>""", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    # Tectonic fault lines (simplified paths)
    ...
    fig_map.add_trace(go.Scattergeo(
        lat=fault_lats, lon=fault_lons, mode="lines",
        line=dict(color="rgba(255,68,0,0.4)", width=1, dash="dash"),
        name="Fault Lines", showlegend=False,
    ))

    # Quake markers
    for q in QUAKES:
        c = mag_color(q["mag"])
        r = int(c[1:3],16); g2 = int(c[3:5],16); b2 = int(c[5:7],16)
        fig_map.add_trace(go.Scattergeo(
            lat=[q["lat"]], lon=[q["lng"]],
            mode="markers",
            marker=dict(
        ...
        fig_map.add_trace(go.Scattergeo(
            lat=[q["lat"]], lon=[q["lng"]],
            mode="markers",
            marker=dict(size=max(8, q["mag"]*4)+8, color="rgba(0,0,0,0)",
    ...
    fig_map.update_geos(
        ...
        showland=True, landcolor="#0c1a0c",
        showocean=True, oceancolor="#06100a",
        showcoastlines=True, coastlinecolor="#142014", coastlinewidth=0.6,
        showframe=False, bgcolor=CARD,
        showlakes=False,
        showrivers=False,
    ...
    for i, q in enumerate(QUAKES[:4]):
        c = mag_color(q["mag"])
        qcols[i].markdown(f"""
        <div style="background:#0f0f0f;border:1px solid {c}33;border-radius:7px;padding:10px 12px">
    ...
    with p2:
        ...
        if run_pred:
            ...
            fig_g = go.Figure(go.Indicator(
                ...
                gauge=dict(
                    axis=dict(range=[0,100], tickcolor=MUTED, tickfont=dict(size=8,color=MUTED)),
                    bar=dict(color=rc, thickness=0.7),
                    bgcolor=BORDER,
                    borderwidth=0,
                    steps=[
                        dict(range=[0,40],  color="#0a0800"),
                        dict(range=[40,70], color="#100800"),
                        dict(range=[70,100],color="#180800"),
                    ],
            ...
            fig_g.update_layout(**{k:v for k,v in plotly_dark(margin=dict(l=20,r=20,t=30,b=10)).items() if k not in ("xaxis","yaxis")},
                                height=220)
            ...
            for label, val, col in [
                ("Aftershock Probability", f"{aft}%",    ORANGE),
                ("Model Confidence",        f"{conf}%",  AMBER),
                ("Predicted Intensity",     intens,      YELLOW),
                ("Impact Radius",           f"{rad} km", TEXT),
            ]:
                st.markdown(f"""
                <div style="display:flex;justify-content:space-between;align-items:center;
                            padding:6px 10px;background:#0a0a0a;border-radius:4px;margin-bottom:3px;
                            border:1px solid #1e0a00;font-size:11px">
                  <span style="color:#5a3a1a">{label}</span>
                  <span style="color:{col};font-weight:700">{val}</span>
                </div>""", unsafe_allow_html=True)

            st.markdown(f"""
            <div style="padding:7px 10px;background:#0d0900;border-radius:5px;margin-top:7px;border:1px solid #2a1400">
              <div style="color:{RED};font-size:9px;font-weight:700">ML: Random Forest + XGBoost Ensemble</div>
              <div style="color:#2a1400;font-size:8px;margin-top:2px">USGS 1900–2024 · 847,293 events · Accuracy: 84.3%</div>
            </div>""", unsafe_allow_html=True)
            ...
            st.markdown(f"""
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:240px;gap:10px">
              <div style="font-size:34px;color:#2a1200">⚡</div>
              <div style="color:#4a2a0a;font-size:12px;text-align:center">Configure parameters and run the model</div>
            </div>""", unsafe_allow_html=True)
...
with tabs[3]:
    ...
    fig_lstm = go.Figure()
    fig_lstm.add_trace(go.Scatter(x=days, y=upper, fill=None, mode="lines",
                                  line=dict(color="rgba(217,119,6,0.3)", width=1, dash="dot"), name="Upper CI", showlegend=False))
    fig_lstm.add_trace(go.Scatter(x=days, y=lower, fill="tonexty", mode="lines",
                                  line=dict(color="rgba(217,119,6,0.3)", width=1, dash="dot"),
                                  fillcolor="rgba(217,119,6,0.08)", name="Confidence Band"))
    fig_lstm.add_trace(go.Scatter(x=days, y=base, mode="lines+markers",
                                  line=dict(color=ORANGE, width=2.5),
                                  marker=dict(color=ORANGE, size=6),
                                  name="Predicted Magnitude"))
    fig_lstm.add_hline(y=6.0, line_color=RED, line_width=1.5, line_dash="dash",
                       annotation_text="Alert Threshold M6.0", annotation_font_color=RED, annotation_font_size=9)
    fig_lstm.update_layout(**plotly_dark(), height=280, legend=dict(font=dict(size=9,color=MUTED)))
    ...
    with s1:
        ...
        for k,v,c in [
            ("Peak Forecast",    f"M{max(base):.1f}", RED),
            ("Forecast Window",  "14 days",           TEXT),
            ("Model Confidence", "79%",               AMBER),
            ("Threshold Breach", f"{breach} days",    ORANGE),
            ("LSTM Layers",      "3 stacked",         TEXT),
            ("Lookback Window",  "30 days",           TEXT),
        ]:
            st.markdown(f"""
            <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #1a0800;font-size:10px">
              <span style="color:#5a3a1a">{k}</span><span style="color:{c};font-weight:700">{v}</span>
            </div>""", unsafe_allow_html=True)
    with s2:
        st.markdown('<div class="card-title">MODEL ARCHITECTURE</div>', unsafe_allow_html=True)
        arch = [("Input Layer","30-day window, 6 features"),("LSTM Layer 1","128 units, dropout 0.2"),
                ("LSTM Layer 2","64 units, dropout 0.2"),("LSTM Layer 3","32 units"),
                ("Dense Layer","16 units, ReLU"),("Output Layer","1 unit, Linear"),
                ("Optimizer","Adam, lr=0.001"),("Loss Fn","Mean Squared Error")]
        for k,v in arch:
            st.markdown(f"""
            <div style="display:flex;gap:8px;font-size:9px;margin-bottom:2px">
              <span style="color:#3a2010;width:90px;flex-shrink:0">{k}</span>
              <span style="color:#a07040">{v}</span>
            </div>""", unsafe_allow_html=True)
...
with tabs[4]:
    ...
    with v1:
        ...
        xs = list(range(80))
        p_wave = [math.sin(i*0.4)*25 + math.sin(i*0.9)*12 + random.gauss(0,4) for i in xs]
        s_wave = [math.cos(i*0.25)*18 + math.sin(i*0.6)*9 + random.gauss(0,3)  for i in xs]
        fig_w2 = go.Figure()
        fig_w2.add_trace(go.Scatter(x=xs, y=p_wave, mode="lines", name="P-Wave",
                                    line=dict(color=ORANGE, width=1.8)))
        fig_w2.add_trace(go.Scatter(x=xs, y=s_wave, mode="lines", name="S-Wave",
    ...
    for i,(name,pres,col) in enumerate(plates):
        ...
        fig_g2.update_layout(**{k:v for k,v in plotly_dark(margin=dict(l=10,r=10,t=20,b=10)).items() if k not in ("xaxis","yaxis")},
                             height=140)
    ...
    tremors = [
        (NOW_MS-600000,   2.1, "Pacific Ocean"),
        (NOW_MS-1800000,  4.3, "Japan Trench"),
        (NOW_MS-3600000,  3.7, "Cascadia Zone"),
        (NOW_MS-7200000,  5.9, "Andes, Peru"),
        (NOW_MS-10800000, 2.4, "Iceland Ridge"),
        (NOW_MS-14400000, 6.8, "Tokyo, Japan"),
        (NOW_MS-21600000, 3.2, "Turkey Fault"),
        (NOW_MS-43200000, 5.1, "Philippines Sea"),
    ]
    for ts, m, place in tremors:
        c = mag_color(m)
        st.markdown(f"""
        <div style="display:flex;align-items:center;gap:10px;padding:5px 0;border-bottom:1px solid #1a0800;font-size:10px">
          <div style="width:8px;height:8px;border-radius:50%;background:{c};flex-shrink:0"></div>
          <div style="color:#5a3a1a;width:60px">{time_ago(ts)}</div>
          <div style="color:{c};font-weight:700;width:34px">M{m}</div>
          <div style="flex:1">{place}</div>
          <div style="color:{c};font-size:9px;width:70px">{mag_label(m)}</div>
        </div>""", unsafe_allow_html=True)
...
with tabs[5]:
    ...
    with e1:
        ...
        fig_dg = go.Figure(go.Indicator(
            ...
            gauge=dict(
                ...
                bar=dict(color=dc, thickness=0.75),
                bgcolor=BORDER, borderwidth=0,
                steps=[
                    dict(range=[0,40],  color="#0a0800"),
                    dict(range=[40,70], color="#100800"),
        ...
        fig_dg.update_layout(**{k:v for k,v in plotly_dark(margin=dict(l=20,r=20,t=30,b=10)).items() if k not in ("xaxis","yaxis")},
                             height=240)
        ...
        if bc1.button("🔴 M7+ WARN"):
            st.warning("MAGNITUDE 7+ EARTHQUAKE DETECTED. SEEK SHELTER IMMEDIATELY. TSUNAMI WARNING IN EFFECT.")
        if bc2.button("🟠 M5+ ALERT"):
            st.warning("MODERATE EARTHQUAKE DETECTED. REMAIN ALERT. AFTERSHOCKS POSSIBLE.")
        if bc3.button("🟢 ALL CLEAR"):
            st.success("ALL CLEAR. SEISMIC ACTIVITY HAS SUBSIDED. RESUME NORMAL OPERATIONS.")
        ...
        if siren_toggle != st.session_state.siren:
            ...
            st.rerun()
        if st.session_state.siren:
            st.markdown(f'<div style="color:{RED};font-size:11px;font-weight:700;animation:pulseDot 1s infinite">🔴 SIREN ACTIVE — BROADCASTING EMERGENCY ALERTS</div>', unsafe_allow_html=True)
    ...
    with e2:
        ...
        fig_ev = go.Figure(go.Indicator(
            ...
            gauge=dict(
                axis=dict(range=[0,100], tickfont=dict(size=8,color=MUTED)),
                bar=dict(color=AMBER, thickness=0.75),
                bgcolor=BORDER, borderwidth=0,
            ),
        ))
        fig_ev.update_layout(**{k:v for k,v in plotly_dark(margin=dict(l=20,r=20,t=30,b=10)).items() if k not in ("xaxis","yaxis")},
                             height=200)
        st.plotly_chart(fig_ev, width='stretch', config={"displayModeBar":False})

        ec1, ec2 = st.columns(2)
        for col,(lbl,val,col2) in zip([ec1,ec2,ec1,ec2],[
            ("Routes Clear","84%",GREEN), ("Shelters Ready","61%",AMBER),
            ("Comms Active","92%",GREEN), ("Med Units","47%",ORANGE),
        ]):
            col.markdown(f"""
            <div style="background:#0a0a0a;border:1px solid #1e0a00;border-radius:5px;padding:8px;text-align:center;margin-bottom:6px">
              <div style="color:#5a3a1a;font-size:9px">{lbl}</div>
              <div style="color:{col2};font-weight:700;font-size:15px">{val}</div>
            </div>""", unsafe_allow_html=True)

        st.markdown('<div class="card-title" style="margin-top:8px">ACTIVE ALERT ZONES</div>', unsafe_allow_html=True)
        alert_zones = [
            ("Bali, Indonesia","CRITICAL",7.2,RED,"30m ago"),
            ("Tokyo, Japan","SEVERE",6.8,ORANGE,"2h ago"),
            ("New Delhi, India","HIGH",5.8,AMBER,"5h ago"),
            ("Rome, Italy","MODERATE",5.5,YELLOW,"15m ago"),
        ]
        for name,level,mag,c,t in alert_zones:
            st.markdown(f"""
            <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;
                        background:{c}10;border:1px solid {c}44;border-radius:5px;margin-bottom:5px">
              <div class="pulse" style="width:9px;height:9px;border-radius:50%;background:{c};flex-shrink:0"></div>
              <div style="flex:1">
                <div style="font-size:10px;font-weight:600">{name}</div>
                <div style="font-size:9px;color:#5a3a1a">M{mag} · {t}</div>
              </div>
              <span style="color:{c};font-size:8px;font-weight:700;border:1px solid {c};border-radius:3px;padding:2px 6px">{level}</span>
            </div>""", unsafe_allow_html=True)
...
with tabs[6]:
    ...
    with an2:
        ...
        fig_yt = go.Figure(go.Scatter(
            x=YEARLY_LABELS, y=YEARLY_DATA, mode="lines+markers",
            line=dict(color=ORANGE, width=2.5),
            marker=dict(color=ORANGE, size=6),
    ...
    with an3:
        ...
        regions = ["Pacific Ring","Himalayan","Mediterr.","Atlantic","E. Africa"]
        counts  = [890, 342, 218, 165, 134]
        cols_r  = [RED, ORANGE, AMBER, YELLOW, "#ca8a04"]
        fig_rb  = go.Figure(go.Bar(y=regions, x=counts, orientation="h",
                                   marker_color=cols_r,
    ...
    with an4:
        ...
        hreg = ["Pacific","Himalaya","Mediterr.","Atlantic","E.Africa"]
        hmon = ["Jan","Mar","May","Jul","Sep","Nov"]
        hdata= np.random.default_rng(7).integers(10, 100, size=(5,6))
        fig_hm = go.Figure(go.Heatmap(
            z=hdata, x=hmon, y=hreg,
            colorscale=[[0,"#0a1a0a"],[0.25,"#eab308"],[0.6,"#ea580c"],[1,"#dc2626"]],
            showscale=False,
            text=hdata, texttemplate="%{text}",
            textfont=dict(size=8, color=TEXT),
        ))
        fig_hm.update_layout(**{k:v for k,v in plotly_dark().items() if k not in ("xaxis","yaxis")},
                             height=230,
    ...
    st.dataframe(df_rec.set_index("Rank"), use_container_width=True,
                 column_config={"Magnitude": st.column_config.TextColumn(width="small"),
                                "Deaths": st.column_config.TextColumn(width="medium")})
...
with tabs[7]:
    ...
    with sl:
        ...
        for s in SENSORS:
            ...
            st.markdown(f"""
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #1a0800;font-size:10px">
              <span class="sensor-dot" style="background:{c}"></span>
              <div style="flex:1">
                <div style="font-weight:700;font-size:10px">{s["name"]}</div>
                <div style="color:#5a3a1a;font-size:9px">{s["loc"]}</div>
              </div>
              <div style="text-align:right">
                <div style="color:{c};font-size:9px;font-weight:700">{s["status"]}</div>
                <div style="color:#3a2010;font-size:8px">{s["lat"]}</div>
              </div>
            </div>""", unsafe_allow_html=True)
    ...
    with sr:
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            values=[1743, 68, 36],
            hole=0.65,
            marker=dict(colors=[GREEN, YELLOW, RED], line=dict(color=CARD, width=2)),
            textinfo="none",
        ))
...
with tabs[8]:
    ...
    with nc2:
        ...
        if st.button("+ TEST ALERT", key="test_notif"):
            st.session_state.notifications.insert(0,{
                "type":"TEST ALERT","msg":"Simulated M5.3 event — Test Region",
                "color":ORANGE,"time":datetime.now().strftime("%H:%M:%S")
            })
            st.rerun()
    ...
    if not st.session_state.notifications:
        ...
        for i, n in enumerate(st.session_state.notifications):
            ...
            with cols[0]:
                st.markdown(f"""
                <div class="notif-item" style="border-color:{c}44;background:{c}10">
                  <div style="width:8px;height:8px;border-radius:50%;background:{c};flex-shrink:0;margin-top:3px"></div>
                  <div style="flex:1">
                    <div style="color:{c};font-size:9px;font-weight:700">{n["type"]}</div>
                    <div style="font-size:10px;margin-top:1px">{n["msg"]}</div>
                    <div style="color:#5a3a1a;font-size:8px;margin-top:1px">{n["time"]}</div>
                  </div>
                </div>""", unsafe_allow_html=True)
            with cols[1]:
...
with tabs[9]:
    ...
    with adm_c2:
        # Download report
        report_lines = [
            "SEISWATCH SEISMIC INTELLIGENCE REPORT",
            f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}",
            "", "=== ACTIVE EVENTS ===",
            *[f"M{q['mag']} | {q['place']} | Depth {q['depth']}km | {time_ago(q['time'])}" for q in QUAKES],
            "", "=== RISK ZONES ===",
            *[f"{z['region']}: {z['risk']}% ({z['trend']})" for z in RISK_ZONES],
            "", "=== ML MODEL STATUS ===",
        ...
        st.download_button("⬇ REPORT", "\n".join(report_lines),
                           file_name=f"seiswatch_report_{int(time.time())}.txt",
        ...
        st.download_button("⬇ CSV", buf.getvalue(),
                           file_name=f"seiswatch_data_{int(time.time())}.csv",
    ...
    with log_col:
        ...
        for l in ADMIN_LOG:
            st.markdown(f"""
            <div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid #1a0800;font-size:9px">
              <span style="color:#3a2010;width:58px;flex-shrink:0">{l["time"]}</span>
              <span style="flex:1">{l["event"]}</span>
              <span style="color:{l["color"]};font-size:8px;font-weight:700;flex-shrink:0">{l["level"]}</span>
            </div>""", unsafe_allow_html=True)
    ...
    with ml_col:
        ...
        def hex_to_rgba(h, a=0.08):
            r2,g2,b2 = int(h[1:3],16), int(h[3:5],16), int(h[5:7],16)
            return f"rgba({r2},{g2},{b2},{a})"

        for name, vals, col in [
            ("Random Forest",[82,79,84,81,90,75], ORANGE),
            ("XGBoost",      [86,88,82,85,78,82], YELLOW),
            ("LSTM",         [78,75,81,78,60,88], RED),
        ]:
...
with tabs[10]:
    ...
    # Suggestion pills
    suggestions = ["What causes earthquakes?","Ring of Fire risks","P vs S wave differences",
                   "Tsunami warning signs","Aftershock predictions","Earthquake preparedness",
                   "Richter vs moment magnitude","How does LSTM forecast seismic activity?"]
            ...
            with st.spinner("Analyzing seismic data..."):
                try:
                    ...
                    response = client.messages.create(
                        ...
                        max_tokens=400,
                        system=(
                            "You are SEISMIC AI, an expert earthquake and geological intelligence assistant "
                            "for an emergency monitoring platform. Deep expertise in seismology, tectonic plates, "
                            "P/S waves, USGS data, tsunami risk, LSTM forecasting, and emergency response. "
                            "Be concise (2-4 sentences), technical but accessible. Plain text only."
                        ),
                        messages=[
...
# ── Footer ───────────────────────────────────────────────────────────────────
st.markdown(f"""
<div style="border-top:1px solid #1a0800;padding:8px 0;margin-top:16px;display:flex;justify-content:space-between;font-size:8px;color:#2a1200">
  <span>SEISWATCH v3.0 · Data: USGS, EMSC, JMA · {datetime.now().year}</span>
  <span>FOR DEMONSTRATION PURPOSES ONLY · NOT FOR EMERGENCY USE</span>
</div>""", unsafe_allow_html=True)
